<template>
  <div class="userTableView" :key="loading">
    <div class="form">
      ...
      <el-button type="primary" :icon="Search" class="form-btn" 
      @click="getPageInfoByInput()">搜索</el-button>
      
      <el-button class="form-btn" plain 
      @click="reGetPageInfo()">重置</el-button>
    </div>
    <div class="form-sec">
        <el-button class="form-btn" type="primary" :icon="Plus"
    ...
    <div class="dialog-upload">
      <el-dialog v-model="dialogUploadVisible" title="上传要导入的表格文件">
        <el-upload
          class="upload-demo"
          ref="uploadRef"
          action="http://localhost:8000/user/import"
          method="post"
...
function getPageInfo(pageNum : number, pageSize : number , username : string , email : string , phone : string) {
  getPage(pageNum, pageSize, username, email, phone).then((res) => {
    if(res.code === '200'){
