<template>
  <div class="userInfo">
    <div class="userInfo-wrap">
      <el-form :label-position="labelPosition" label-width="100px" :model="formUserInfo" style="max-width: 25vw">
        <el-row :gutter="20">
    ...
    <div class="project-wrap">
      ...
      <el-table ref="tableRef" row-key="start" :data="tableData" style="max-width: 50vw" height="246" max-height="300">
        ...
        <el-table-column prop="start" label="项目开始时间" sortable width="180" column-key="start" />
        <el-table-column prop="end" label="项目结束时间" sortable width="180" column-key="end" />
      ...
      <div class="form-table">
        <el-table :data="collectData" @current-change="" :border="true" ref="singleTableRef" height="350">
          <el-table-column label="收藏夹封面" width="100" align="center" header-align="center">
            <template #default="scope">
              <el-image style="width: 75px; height: 75px;" :src="scope.row.cover" :zoom-rate="1.2" lazy fit="cover" />
            </template>
          </el-table-column>
    ...
    <div class="dialog-uploadAvatar">
      <el-dialog v-model="dialogUploadAvatar" title="上传要导入的图片文件">
        <el-upload class="upload-demo" ref="uploadAvatar" :action="`${uploadUrl}file/upload`" method="post"
          list-type="picture" :on-exceed="handleExceed" :on-success="handleUploadSuccess" :on-error="handleUploadError"
...
async function submitForm(FormData: any) {
  const [err, res] = await to(addOrUpdateUser(FormData));
  if (err) {
    remindElMessage('更新失败', 2)
  } else {
    // 获取当前用户的id，进行判断
...
async function getCurrentUserInfo() {
  ...
  if (userId != null) {
    const [err, res] = await to(getTodoListByUserId(userId));
    if (err) {
      remindElMessage('获取数据报错', 2)
    } else {
      todoList.value = res?.data
    }

    const [err2, res2] = await to(getProjectByUserId(userId));
    if (err2) {
      remindElMessage('获取数据报错', 2)
    } else {
...
const handleExceed: UploadProps['onExceed'] = (files) => {
  remindElMessage('只能上传一个文件')
}

// 上传成功函数
function handleUploadSuccess() {
  remindElMessage('上传成功')
